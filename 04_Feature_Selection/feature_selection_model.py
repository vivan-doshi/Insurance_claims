...
n_splits_cv = 5
# number of bins we are creating for stratification
n_bins_stratify = 4 
# top feature count per target (for Method 1)
n_top_features_per_target = 26 
# feature importance threshold (relative) (for Method 2)
...
# (Importance calculation, RFECV calculation, and plotting code remains the same)
...
for target_name in target_variables:
    ...
    y_target = Y_train_dict[target_name]; cv_iterator = None; model = None; stratify_values = None
    if target_name == 'Claim_Status':
        ...
        try:
            non_zero_mask = y_target > 0
            if non_zero_mask.sum() > n_bins_stratify and y_target[non_zero_mask].nunique() >= n_bins_stratify:
                 stratify_values = pd.qcut(y_target[non_zero_mask], q=n_bins_stratify, labels=False, duplicates='drop') + 1
                 stratify_values = pd.Series(stratify_values, index=y_target[non_zero_mask].index).reindex(y_target.index, fill_value=0).astype(int)
                 # print(f"Using {stratify_values.nunique()} bins for stratification (incl. zeros).") # Less verbose
            else: stratify_values = (y_target > 0).astype(int); # print(f"Using 0 vs >0 stratification.")
            if stratify_values.nunique() < 2: print(f"Warning: Cannot stratify {target_name}. Using KFold."); cv_iterator = KFold(n_splits=n_splits_cv, shuffle=True, random_state=42).split(X_train_processed)
            else: cv_iterator = skf.split(X_train_processed, stratify_values)
        except Exception as e_bin: print(f"Error binning {target_name}: {e_bin}. Using KFold."); cv_iterator = KFold(n_splits=n_splits_cv, shuffle=True, random_state=42).split(X_train_processed)
    ...
    try:
        for fold, (train_index, val_index) in enumerate(cv_iterator):
            ...
            model.fit(X_fold_train, y_fold_train); fold_importances[f'fold_{fold+1}'] = model.feature_importances_
        ...
        top_n = importance_df['feature'].head(n_top_features_per_target).tolist(); all_top_features_per_target[target_name] = top_n
        ...
        threshold_list = importance_df[importance_df['average_importance'] > abs_thresh]['feature'].tolist(); all_threshold_features_per_target[target_name] = threshold_list
    ...
    except Exception as e: print(f"Error calculating importance for {target_name}: {e}"); all_top_features_per_target[target_name] = []; all_threshold_features_per_target[target_name] = []; all_importance_dfs[target_name] = pd.DataFrame()

# == Combine features & Plot for Methods 1 & 2 ==
combined_top_n_set = set().union(*all_top_features_per_target.values()); final_union_top_n_features = sorted(list(combined_top_n_set))
combined_threshold_set = set().union(*all_threshold_features_per_target.values()); final_union_threshold_features = sorted(list(combined_threshold_set))
...
if all_importance_dfs:
    ...
    for target_name, imp_df in all_importance_dfs.items():
         if not imp_df.empty: combined_importance = combined_importance.merge(imp_df.set_index('feature'), left_index=True, right_index=True, how='left'); combined_importance.rename(columns={'average_importance': f'importance_{target_name}'}, inplace=True)
    ...
    if importance_cols:
        ...
        plt.barh(top_importances.index, top_importances['overall_avg_importance'], align='center'); plt.xlabel('Overall Average Importance (Gain)'); plt.ylabel('Feature'); plt.title(f'Top {n_features_to_plot} Features by Overall Average Importance'); plt.gca().invert_yaxis(); plt.tight_layout(); plt.show()
    else: print("Could not calculate overall importance.")
else: print("No importance dataframes available to generate plot.")
...
for target_name in target_variables:
    ...
    # --- Determine estimator, scorer, and stratification values ---
    if target_name == 'Claim_Status':
        ...
        # Determine stratification bins for LC/HALC
        try:
            non_zero_mask = y_target > 0
            if non_zero_mask.sum() > n_bins_stratify and y_target[non_zero_mask].nunique() >= n_bins_stratify:
                 stratify_values_rfecv = pd.qcut(y_target[non_zero_mask], q=n_bins_stratify, labels=False, duplicates='drop') + 1
                 stratify_values_rfecv = pd.Series(stratify_values_rfecv, index=y_target[non_zero_mask].index).reindex(y_target.index, fill_value=0).astype(int)
                 print(f"Using {stratify_values_rfecv.nunique()} bins for RFECV stratification.")
            else:
                 stratify_values_rfecv = (y_target > 0).astype(int)
                 print(f"Using 0 vs >0 stratification for RFECV.")
        except Exception as e_bin_rfe:
            print(f"Error during binning for RFECV target {target_name}: {e_bin_rfe}. Using 0 vs >0 stratification.")
            stratify_values_rfecv = (y_target > 0).astype(int)

    # --- Generate CV Splits ---
    if stratify_values_rfecv is not None and stratify_values_rfecv.nunique() >= 2:
        print(f"Generating StratifiedKFold splits based on computed bins/values.")
        skf_rfecv = StratifiedKFold(n_splits=n_splits_cv, shuffle=True, random_state=42)
        cv_splits = list(skf_rfecv.split(X_train_processed, stratify_values_rfecv))
    else:
...
# --- (Rest of the script: Plotting, Summary, Saving remains the same) ---
...
for target_name, selector in rfecv_selectors.items():
    try:
        if hasattr(selector, 'cv_results_') and 'mean_test_score' in selector.cv_results_:
            cv_scores = selector.cv_results_['mean_test_score']; n_features_tested = len(cv_scores)
            features_range = range(selector.min_features_to_select, selector.min_features_to_select + n_features_tested)
            plt.figure(figsize=(8, 5)); plt.xlabel("Number of Features Selected"); plt.ylabel(f"CV Score ({selector.scoring})"); plt.title(f"RFECV Performance for {target_name}")
            plt.plot(features_range, cv_scores); plt.grid(True)
            if hasattr(selector, 'n_features_') and (selector.n_features_ - selector.min_features_to_select) >= 0 and (selector.n_features_ - selector.min_features_to_select) < len(cv_scores):
                 plt.plot(selector.n_features_, selector.cv_results_['mean_test_score'][selector.n_features_ - selector.min_features_to_select], 'ro', markersize=8, label=f'Optimal ({selector.n_features_})')
                 plt.legend()
            plt.show()
        else: print(f"No valid cv_results_ found for {target_name} RFECV.")
    except Exception as e: print(f"Could not plot RFECV results for {target_name}: {e}")
...
chosen_features = []; method_applied = "None"
# (Logic remains the same)
if final_selection_method_to_apply == 'lgbm_union_top_n':
    chosen_features = final_union_top_n_features; method_applied = final_selection_method_to_apply
elif final_selection_method_to_apply == 'lgbm_union_threshold':
    chosen_features = final_union_threshold_features; method_applied = final_selection_method_to_apply
    ...
    if target_for_rfecv_apply in rfecv_features_per_target:
        chosen_features = rfecv_features_per_target[target_for_rfecv_apply]; method_applied = final_selection_method_to_apply
    else: print(f"Warning: RFECV results for '{target_for_rfecv_apply}' not found."); method_applied = "None"
elif final_selection_method_to_apply == 'none': method_applied = "None"
else: print(f"Warning: Unknown selection method '{final_selection_method_to_apply}'."); method_applied = "None"
...
if method_applied != "None" and chosen_features:
    ...
    X_train_final = X_train_processed[chosen_features]; X_test_final = X_test_processed[chosen_features]
    print(f"Saving filtered training features to: {output_train_path}"); X_train_final.to_csv(output_train_path, index=True)
    print(f"Saving filtered test features to: {output_test_path}"); X_test_final.to_csv(output_test_path, index=True)
...
elif method_applied != "None" and not chosen_features: print(f"Warning: Chosen method '{method_applied}' resulted in empty list. No data saved.")
else: print("Selection method set to 'none' or chosen method failed. No filtered data saved.")
